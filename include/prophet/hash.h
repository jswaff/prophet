 ...
 *
 * Probe the main hash table.  The index used is computed by taking the key argument modulo the table capacity.
 *
 ...
 *
 * Probe the pawn hash table.  The index used is computed by taking the key argument modulo the table capacity.
 *
 * The internal "probes" counter is incremented with every probe.  If an entry is found, either the "hits" or 
 * "collisions" counter is incremented depending on whether the full key comparison check passed.
 *
