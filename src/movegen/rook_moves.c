...
// TODO: this is wasteful.  use a one dimensional array and precompute the offset for each square
...
static void init_occupiers_and_blockers()
{
    for (uint32_t sq=0; sq<64; sq++) {
        ...
        for (uint32_t i=0; i<num_variations; i++) {
            ...
            // the idea is that each bit in the index maps to a bit in the mask
            uint32_t index = i;
            while (index) {
...
static void init_magic_numbers()
{
    for (uint32_t sq=0; sq<64; sq++) {
        ...
        do {
            magic = random64() & random64() & random64(); // approximately 8 bits
            ...
            for (uint32_t i=0; i<num_variations; i++) {
                ...
                // fail if this index is used by a blocker set that is incorrect
        ...
        // occupiers (interior squares reachable from this square), we can derive an index.  Any other combination of
        // occupiers that maps to the same index has the same "blocker set".
