...
static move* add_pawn_move(move* m, square_t from, square_t to, piece_t captured_piece, bool epcapture);
static move* add_promotion(move* m, square_t from, square_t to, piece_t promopiece, piece_t captured_piece);
 ...
 *
 * If the move is a capture then the capture flag is set.  If it is a promotion then four moves are added.
 *
 * \param m             Pointer to a move stack
 * \param from          The square the pawn is moving from
 * \param to            The square the pawn is moving to
 * \param captured_piece   The captured piece.  May be NO_PIECE if not a capture.
