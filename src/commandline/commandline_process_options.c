...
int commandline_process_options(int argc, char* argv[])
{
    ...
    static struct option long_options[] = {
        ...
        {"random starting position", required_argument, 0, 'n'}
    };

    int long_index = 0;
    int opt = 0;
    while ((opt = getopt_long(argc, argv, "lrd:h:p:e:n:", long_options, &long_index)) != -1)
    {
        switch (opt)
        {
            case 'l':
            ...
            case 'n':
                out(stdout, "making %s random moves for both players before starting clocks\n", optarg);
