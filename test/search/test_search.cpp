...
TEST(search_test, depth0_no_bounds)
{
    ...
    ASSERT_EQ(eval(&pos, false), search(&pos, 0, -INF, INF, moves, &stats));
}
...
TEST(search_test, mate_in_1)
{
    ...
    ASSERT_EQ(CHECKMATE-1, search(&pos, 2, -INF, INF, moves, &stats));
}
...
TEST(search_test, mate_in_2)
{
    ...
    ASSERT_EQ(CHECKMATE-3, search(&pos, 4, -INF, INF, moves, &stats));
}
...
TEST(search_test, mate_in_3)
{
    ...
    ASSERT_EQ(CHECKMATE-5, search(&pos, 6, -INF, INF, moves, &stats));
}
...
TEST(search_test, stalemate)
{
    ...
    ASSERT_EQ(0, search(&pos, 1, -INF, INF, moves, &stats));
}
